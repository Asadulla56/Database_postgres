...
CREATE TABLE persons (
    ...
    rating NUMERIC(3, 2) CHECK (
        rating >= 0
        AND rating <= 9.99
    ) -- Adding CHECK constraint to limit the range of numer
);
...
-- 5. renaming a column name
ALTER TABLE person RENAME COLUMN dob to date_of_birth;
...
-- 6. add/drop constraint
alter Table person alter COLUMN profession set not null;

alter Table person alter COLUMN profession drop not null;

-- different syntax for check, PRIMARY key and foreign KEY

alter Table person alter COLUMN dob set UNIQUE;
-- will throw error
...
alter Table person DROP constraint unique_person_dob
...
SELECT * FROM persons WHERE age BETWEEN 30 AND 40;
...
INSERT INTO
    users (user_id, username, email)
VALUES (4, 'Emma', 'emma@example.com'),
    (
        5,
        'Michael',
        'michael@example.com'
...
SELECT * FROM users u FULL JOIN orders o ON u.user_id = o.user_id

-- different syntax
SELECT * FROM users u FULL JOIN orders USING (user_id)
...
SELECT * FROM persons WHERE name LIKE 'J%' AND dob IS NOT NULL;
